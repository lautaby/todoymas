...
export async function POST(request: Request) {
  try {
    ...
    // Use the sandbox URL automatically when a test access token is used
    // (test tokens start with "TEST-"), so payments can be tried end-to-end
    // in development without a live Mercado Pago account.
    const isTestToken = accessToken.startsWith('TEST-');
    const initPoint = isTestToken ? mpData.sandbox_init_point : mpData.init_point;
